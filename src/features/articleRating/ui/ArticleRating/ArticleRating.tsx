...
function ArticleRating({ articleId }: Props) {
	...
	const { data, isLoading, refetch } = useGetRatingArticle(
		{
			userId: user?.id || "",
			articleId,
		},
	...
	return (
		<div>
			<RatingCard onChange={setRating} title={"Оцените статью"} selectedStars={data?.length ? data[0].rate : null} />
		</div>
	)
}
