...
function RatingCard({ title, onChange, selectedStars }: Props) {
	...
	return (
		<div className={s.wrapper}>
			{title && !selectedStars ? <h3 className={s.title}>{title}</h3> : null}
			{title && selectedStars ? <h3 className={s.title}>Спасибо за отзыв!</h3> : null}
			<StarRating onSelect={changeStars} selectedStars={valueRating} />
			<Modal isOpen={isOpenMoadal} onClose={closeMoadal}>
				<div className={s.modal}>
					{title ? <h3 className={s.title}> {title}</h3> : null}
					<AppInput value={valueInput} onChange={changeInput} label="Описание" className={s.input} />
					<div className={s.moadlBtns}>
						<AppButton onClick={sendChange} theme={AppButtonTheme.PRIMARY} disabled={!valueInput.length}>
							Отправить
						</AppButton>
						<AppButton onClick={resetChange} theme={AppButtonTheme.SECONDARY}>
							Отменить
						</AppButton>
					</div>
				</div>
