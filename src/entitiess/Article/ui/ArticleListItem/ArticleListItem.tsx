...
function ArticleListItem({ article, view, isLoading, target = "_parent" }: ArticleListItemProps) {
	...
	const renderTags = () => {
		...
		if (article.type?.length) {
			return (
				<div className={s.articleTitleTags}>
		...
		return (
			...
			>
				...
				<div className={s.articleFotter}>
					<AppButton theme={AppButtonTheme.SECONDARY} disabled={true}>
						Подробнее
					</AppButton>
					<div className={s.articleFotterViews}>
						<Skeleton width={"40px"} height={20} />
		...
		>
			...
			{article.img ? (
				<div className={s.articleImg}>
					<AppImage src={article.img} />
				</div>
			) : null}
			<div className={s.text}>{article.subtitle}</div>
			<div className={s.articleFotter}>
