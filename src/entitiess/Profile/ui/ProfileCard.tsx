...
export const ProfileCard = (props: ProfileCardProps) => {
	const { data, error, readonly, isLoading, validateErrors, createFunChange } = props

	const renderInput = (value: string | number | undefined, onChange: any, key: string) => {
		return <AppInput key={key} label={key} value={value} onChange={onChange} readonly={readonly} />
	}
	...
	const renderCurrency = useMemo(() => {
		...
		return (
			<CurrencySelect value={data?.currency || undefined} onChange={newFunc} label={"currency"} readOnly={readonly} />
		)
